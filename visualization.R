...
ggplot(broadband,aes(y=Town)) +
  ...
  geom_bar(data=filter(broadband,County=="YORKSHIRE" | County=="WEST YORKSHIRE" | County=="SOUTH YORKSHIRE" | 
...
broadband %>% 
  ...
  geom_boxplot() +
  labs(title = "Average download speed (Mbit/s) by County", x = "County",
...
ggplot(Drugs_2021_2022, aes(x = Year, y = DrugOffenceRate, color = County)) +
  ...
  labs(x = "Year", y = "Drug Offense Rate per 10000 People", 
       title = "Drug Offense Rate per 10000 People from 2021 to 2022", color = "County")




#  -Radar chart = vehicle crime rate 2021-2022 (both countries)
...
#- box plot= average attainment score in 2022 both country district
...
#   - line chart = oxfordshire district average attainment score  from 2020 - 2022
...
speeds = read_csv("Cleaning/Cleaned datasets/Cleaned_Broadband_Speed.csv") %>%  na.omit()  
...
#=============drug rate vs average attainment core per 10000 people =======
...
Drugs = crime %>%
  ...
  group_by(Town, County) %>%
  filter(CrimeType == "Drugs") %>% 
  mutate(DrugOffenceRate = (CrimeCount)) %>% 
  as_tibble() %>% 
  na.omit()
...
ggplot(lm_res4, aes(x = meanAttainment / (Population2019 + Population2020 + Population2021 + Population2022) * 10000, y = DrugOffenceRate)) +
  ...
  geom_smooth(method = lm, se = FALSE, color = "peachpuff4") +
  labs(x ="Average Attainment Score per 10000 People", y ="Drug Offense Rate", 
       title ="Average Attainment Score vs Drug Offense Rate", color ="County")
...
ggplot(lm_res5, aes(x = meanAttainment, y = meanDownloadSpeed)) +
  ...
  geom_smooth(method = lm, se = FALSE, color = "peachpuff4") +
  labs(x="Average Attainment Score",y="Average Download Speed",title="Average Attainment Score vs Average Download Speed",
       color="County")
